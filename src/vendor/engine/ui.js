(function (root, factory) {
  ...
  function mount(opts) {
    ...
    function reloadEngines(preferredEngine) {
      if (preferredEngine && deck.engines[preferredEngine]) {
        currentEngine = preferredEngine;
      } else if (!deck.engines[currentEngine]) {
        currentEngine = getPreferredEngine() || deck.defaultEngine;
      }
      inputEl.placeholder = placeholderOverride || deck.placeholderFor(currentEngine);
      renderEngineList();
      refreshEngineDisplay(false);
    }
