(function () {
  ...
  function button(name, i18nKey, title, onClick) {
    ...
    b.setAttribute("data-i18n-aria-label", i18nKey);
  ...
  function mount() {
    ...
    var maximizeBtn = button("maximize", "titlebar.maximize", "Maximize", function () {
    ...
    function syncMaximizeIcon() {
      ...
      current.isMaximized().then(function (isMax) {
        ...
        var key = isMax ? "titlebar.restore" : "titlebar.maximize";
        var fallback = isMax ? "Restore" : "Maximize";
        var label = (window.LumaI18n ? window.LumaI18n.t(key, fallback) : fallback);
    ...
    controls.appendChild(
      button("minimize", "titlebar.minimize", "Minimize", function () {
        if (current) current.minimize();
      })
    );
    controls.appendChild(maximizeBtn);
    controls.appendChild(
      button("close", "titlebar.close", "Close", function () {
